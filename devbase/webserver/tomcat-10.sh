...
Install_Tomcat10() {
  ...
  if [ ! -e "${tomcat_install_dir}/conf/server.xml" ]; then
    ...
    echo "${CFAILURE}Tomcat install failed, Please contact the author1! ${CEND}" && grep -Ew 'NAME|ID|ID_LIKE|VERSION_ID|PRETTY_NAME' /etc/os-release
    kill -9 $$; exit 1;
  fi
  ...
  rm -rf tomcat-native-*
  if [ -e "${apr_install_dir}/lib/libtcnative-2.la" ]; then
...
EOF
    ...
    popd # goto ${ubdevenv_dir}/src
    /bin/cp ${ubdevenv_dir}/config/server.xml ${tomcat_install_dir}/conf
...
EOF
    ...
    /bin/cp ${ubdevenv_dir}/init.d/Tomcat-init /etc/init.d/tomcat
    ...
    popd > /dev/null
    echo "${CFAILURE}Tomcat install failed, Please contact the author2! ${CEND}" && grep -Ew 'NAME|ID|ID_LIKE|VERSION_ID|PRETTY_NAME' /etc/os-release
  fi
