...
log_dir="${ubdevenv_dir}/log/install_devaddons.log"
...
. ./devbase/firewall/fail2ban.sh
. ./devbase/webserver/ngx_lua_waf.sh
. ./devbase/language/php/composer.sh
. ./devbase/language/php/phpmyadmin.sh
...
while :; do
  ...
  case "$1" in
    ...
    --)
      ...
      ;;
    *)
      echo "${CWARNING}ERROR: unknown argument! ${CEND}" && Show_Devaddons_Help && exit 1
      ;;
  esac
done
      ...
      case "${Number}" in
        1)
          ...
          if [ "${install_flag}" = 'y' ]; then
            Install_composer 2>&1 | tee -a $log_dir
        ...
        2)
          ACTION_FUN
          if [ "${install_flag}" = 'y' ]; then
            Install_fail2ban 2>&1 | tee -a $log_dir
          elif [ "${uninstall_flag}" = 'y' ]; then
            Uninstall_fail2ban
          fi
          ;;
        3)
          ...
          if [ "${install_flag}" = 'y' ]; then
            ...
            enable_lua_waf 2>&1 | tee -a $log_dir
          elif [ "${uninstall_flag}" = 'y' ]; then
            disable_lua_waf
          fi
          ;;
        4)
          ACTION_FUN
          if [ "${install_flag}" = 'y' ]; then
            Install_Supervisor 2>&1 | tee -a $log_dir
        ...
        5)
          ACTION_FUN
          if [ "${install_flag}" = 'y' ]; then
            Install_phpMyAdmin 2>&1 | tee -a $log_dir
          elif [ "${uninstall_flag}" = 'y' ]; then
...
if [ ${ARG_NUM} == 0 ]; then
  ...
  if [ "${supervisord_flag}" == 'y' ]; then
    if [ "${install_flag}" = 'y' ]; then
      Install_Supervisor 2>&1 | tee -a $log_dir
      Install_SupervisorDesktop 2>&1 | tee -a $log_dir
    elif [ "${uninstall_flag}" = 'y' ]; then
      Uninstall_Supervisor
      Uninstall_SupervisorDesktop
    fi
