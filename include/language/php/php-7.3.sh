...
Install_PHP73() {
  ...
  #[ "${isOpenSSL3}" = "y" ] && patch -p1 < ${oneinstack_dir}/src/patch/php-7.3-openssl3.0.patch
  ...
  if [ "${Apache_main_ver}" == '22' ] || [ "${apache_mode_option}" == '2' ]; then
    ...
    --enable-mbstring --with-password-argon2 --with-sodium=/usr/local --with-gd --with-openssl-dir=${openssl_install_dir} \
    ...
    --with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-exif \
    --enable-sysvsem --enable-inline-optimization --with-curl=${curl_install_dir} --enable-mbregex \
    --enable-mbstring --with-password-argon2 --with-sodium=/usr/local --with-gd --with-webp-dir --with-openssl-dir=${openssl_install_dir} \
    --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc --enable-ftp --enable-intl --with-xsl \
    --with-gettext --enable-zip --without-libzip --enable-soap --disable-debug ${php_modules_options}
  fi
