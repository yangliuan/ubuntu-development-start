...
log_dir="${start_dir}/log/install_ubsoft.log"
...
. ubsoftware/patch_suport.sh
. ubsoftware/input-method/fcitx.sh

ARG_NUM=$#
TEMP=`getopt -o hvV --long help,version,input_method_option:,baidunetdisk,chrome,deepinwine,dingtalk,linuxqq,feishu,flameshot,indicator_sysmonitor,lantern,neteasy_cloudmusic,qqmusic,peek,qv2ray,sunlogin,theme_tools,bilibili_video_downloader,wps,xDroid,conky,my_weather_indicator,custome,gnome_center,reboot -- "$@" 2>/dev/null`
...
# check custome software
while :; do echo
    read -e -p "Do you want to install custome software? [y/n](y): " custome_flag
    custome_flag=${custome_flag:-y}
    if [[ ! ${custome_flag} =~ ^[y,n]$ ]]; then
        echo "${CWARNING}input error! Please only input 'y' or 'n'${CEND}"
    else
        break;
    fi
...
while :; do echo
    read -e -p "Do you want to reinstall gnome center? [y/n](n): " gnome_center_flag
...
if [ "${indicator_stickynotes_flag}" == 'y' ]; then
    . ubsoftware/indicator_stickynotes.sh
...
if [ "${custome_flag}" == 'y' ]; then
    ...
    Install_Custome_SnapApp 2>&1 | tee -a $log_dir
    Install_Custome_AptApp 2>&1 | tee -a $log_dir
fi

#reinstall gnome center
if [ "${gnome_center_flag}" == 'y' ]; then
    Reinstall_GnomeCenter 2>&1 | tee -a $log_dir
fi
